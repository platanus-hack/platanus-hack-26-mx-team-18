...
import { JALISCO_SOURCES } from "./sources";

async function main() {
  console.log("[jalisco] Scraper placeholder — sin lógica implementada");
  console.log(`[jalisco] Fuentes configuradas: ${JALISCO_SOURCES.length}`);
  JALISCO_SOURCES.forEach((source) => {
    console.log(`  - ${source.name}: ${source.url || "(URL pendiente)"}`);
  });
}

main().catch((err) => {
  console.error("[jalisco] Error:", err);
  process.exit(1);
});
